...
def build_flexible_url(origin: str, destination: str, trip_type: str = "one-way") -> str:
    """URL sem data para busca flexível."""
    if trip_type == "round-trip":
        trip = f"{origin} to {destination} return"
    else:
        trip = f"{origin} to {destination} one way"
    base = BASE_URL.rstrip("/")
    if base.endswith("/travel/flights"):
        base = f"{base}/search"
    elif "/travel/flights/search" not in base:
...
def _extract_calendar_prices(page, notes: list[str]) -> dict[str, float]:
    """Extrai preços do calendário (date picker) do Google Flights.
    Retorna {data_iso: preço}."""
    prices = {}
    # Tenta vários seletores para células do calendário com preço
    selectors = [
        '[aria-label*="R$"]',
        '[aria-label*="R$"]',
        'div[role="gridcell"]',
        ...
        try:
            ...
            for cell in cells:
                try:
                    ...
                    # Padrão: "15 de junho de 2026 R$ 450"
                    price_match = re.search(r'R\$\s*([\d.]+,\d{2})', aria + ' ' + text)
                    date_match = re.search(r'(\d{1,2})\s+de\s+(\w+)\s+de\s+(\d{4})', aria + ' ' + text)
                    if price_match and date_match:
                        ...
                        mn = month_name.lower().rstrip('.,')
                        if mn in month_map:
                            date_key = f'{year}-{month_map[mn]:02d}-{int(day):02d}'
                            prices[date_key] = float(price_str)
                except Exception:
                    continue
            if prices:
                break
...
def _extract_graph_prices(page, notes: list[str]) -> list[dict] | None:
    """Extrai preços do gráfico de barras do Google Flights.
    Clica em 'Gráfico de preços', extrai alturas das barras SVG,
    converte para preços. Retorna [{day, price}] ordenado por dia."""
    try:
        # Click the price graph button
        btn = page.locator('text=Gráfico de preços').first
        if btn.count() == 0:
            notes.append('graph_button_not_found')
            return None
        btn.click()
        time.sleep(2)
        ...
        result = page.evaluate("""() => {
            ...
            if (!bars.length) return {maxPrice: 3000, prices: []};
            // Try to find the max Y-axis label
            let maxPrice = 3000;
            const yLabels = document.querySelectorAll(".oHo7ed text tspan");
            const prices = [];
            yLabels.forEach(t => {
            ...
            bars.forEach(b => {
                ...
                const graphTop = 262, graphHeight = 294;
                const barTop = rect.y - graphTop;
                const price = Math.round((graphHeight - barTop) / graphHeight * maxPrice);
                data.push(Math.round(price));
            });
            return {maxPrice, prices: data};
        }""")
        
        if not result or not result.get('prices'):
            notes.append('graph_no_bars_found')
            return None
        
        max_price = result.get('maxPrice', 3000)
        notes.append(f'graph_max_price={max_price}')
        
        # Convert to [{day, price}]
        days = []
        for i, price in enumerate(result['prices']):
            days.append({'day': i + 1, 'price': price})
        
        notes.append(f'graph_extracted_{len(days)}_days')
        return days
    except Exception as e:
        notes.append(f'graph_extract_error={str(e)[:80]}')
        return None


def _open_date_picker(page, notes: list[str]) -> bool:
    """Abre o date picker clicando no campo de data."""
    date_selectors = [
        ...
        'div[aria-label*="Data"]',
        '.VfPpkd-fmcmS-Ysl6R-haAclf',  # common date button class
    ]
    for sel in date_selectors:
        try:
            ...
            if el.is_visible(timeout=2000):
                el.click()
                time.sleep(1.5)
                notes.append(f'date_picker_opened_via={sel[:40]}')
                return True
        except Exception:
            continue
    return False
...
def _is_flexible_mode() -> bool:
    return os.getenv('GOOGLE_FLIGHTS_FLEXIBLE', '0') in {'1', 'true', 'yes', 'on'}


def _get_flexible_month() -> str:
    return os.getenv('GOOGLE_FLIGHTS_MONTH', '')
...
def maybe_open_booking(page, summary_price: float | None, notes: list[str], allow_agencies: bool = False, is_international: bool = False, booking_url_fallback: list | None = None) -> tuple[bool, str, float | None, float | None, list[dict], str, tuple[str, float, float | None, list[dict], str] | None, tuple[str, float, float | None, list[dict], str] | None, str]:
    ...
    booking_timeout_ms = BOOKING_CONTENT_TIMEOUT_MS if is_international else 25000
...
def run(origin: str, destination: str, outbound_date: str, inbound_date: str = "") -> dict:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # ── Modo flexível: sobrescreve outbound_date e usa date picker ──
            flexible_mode = _is_flexible_mode()
            flexible_result = None
            if flexible_mode:
                flexible_month = _get_flexible_month()
                trip_type = os.getenv('GOOGLE_FLIGHTS_TRIP_TYPE', 'one-way')
                notes.append(f'flexible_mode=trip_type={trip_type} month={flexible_month}')
                if trip_type == 'round-trip':
                    flexible_result = _run_flexible_roundtrip(origin, destination, flexible_month, page, context, browser, notes)
                else:
                    flexible_result = _run_flexible_oneway(origin, destination, flexible_month, page, context, browser, notes)
                if flexible_result and flexible_result.get('outbound_date'):

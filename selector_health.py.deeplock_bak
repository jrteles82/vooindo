#!/home/teles/dev/python/skyscanner-bot/.venv/bin/python
"""
...
SELECTOR_REGISTRY: list[dict[str, Any]] = [
    # ── área principal ───────────────────────────────────────────────────────
    {
        ...
        "files": ["google_flights_executor.py", "skyscanner.py", "update_scraper.py"],
        "critical": True,
        "kind": "css",
    },
    # ── cards de voo ─────────────────────────────────────────────────────────
    ...
    },
    {
        ...
        "files": ["google_flights_executor.py", "skyscanner.py"],
        "critical": False,
        "kind": "css",
    },
    {
        ...
        "files": ["skyscanner.py", "update_scraper.py"],
        "critical": False,
        "kind": "css",
    },
    {
    ...
    # ── autenticação / perfil ────────────────────────────────────────────────
    {
        ...
        "files": ["google_flights_executor.py", "skyscanner.py", "renew_google_session.py"],
        "critical": False,
        "kind": "css",
    },
    {
    ...
    },
    {
        ...
        "files": ["update_scraper.py", "skyscanner.py"],
        "critical": False,
        "kind": "text",
    },
    {
...
def _autocorrect_via_gpt(page, entry: dict[str, Any]) -> str | None:
    try:
        from openai import OpenAI  # noqa: PLC0415
    except ImportError:
        logger.warning("openai não instalado — GPT desabilitado")
        return None
    api_key = os.getenv("OPENAI_API_KEY", "").strip()
    if not api_key:
        logger.warning("OPENAI_API_KEY não configurado — GPT desabilitado")
        return None
    dom_html = _capture_dom_html(page)
    if not dom_html:
        logger.error("Não foi possível capturar o DOM para GPT")
        return None
    prompt = _build_llm_prompt(entry, dom_html)
    try:
        client = OpenAI(api_key=api_key)
        response = client.responses.create(
            model=os.getenv("SELECTOR_HEALTH_GPT_MODEL", "gpt-5-mini"),
            input=prompt,
            max_output_tokens=150,
        )
        suggestion = (getattr(response, "output_text", "") or "").strip().strip("\"'`")
        logger.info("GPT sugeriu: '%s'", suggestion)
        return suggestion or None
    except Exception as exc:
        logger.error("Erro na chamada GPT API: %s", exc)
        return None


def _autocorrect_via_claude(page, entry: dict[str, Any]) -> str | None:
    try:
        import anthropic  # noqa: PLC0415
    except ImportError:
        logger.warning("anthropic não instalado — Claude desabilitado")
        return None
    api_key = os.getenv("ANTHROPIC_API_KEY", "").strip()
    if not api_key:
        logger.warning("ANTHROPIC_API_KEY não configurado — Claude desabilitado")
        return None
    dom_html = _capture_dom_html(page)
    if not dom_html:
        logger.error("Não foi possível capturar o DOM para Claude")
        return None
    prompt = _build_llm_prompt(entry, dom_html)
    try:
        client = anthropic.Anthropic(api_key=api_key)
        response = client.messages.create(
            model=os.getenv("SELECTOR_HEALTH_CLAUDE_MODEL", "claude-sonnet-4-6"),
            max_tokens=150,
            messages=[{"role": "user", "content": prompt}],
        )
        suggestion = response.content[0].text.strip().strip("\"'`")
        logger.info("Claude sugeriu: '%s'", suggestion)
        return suggestion or None
    except Exception as exc:
        ...
        with sync_playwright() as pw:
            ...
            for entry in SELECTOR_REGISTRY:
                ...
                for source_name, resolver in [
                    ...
                    ("gpt", _autocorrect_via_gpt),
                    ("claude", _autocorrect_via_claude),
                ]:
